...
def new_file(code = 0):
    os.startfile("{0}\\TextPad.pyw".format(os.getcwd()))
...
def showabout():
    ...
    about_txt_2 = tk.Label(about, text="This Project has started in Jan 24, 2022. Programmed in Python fully.", bg="white")
    header.pack(pady=20)
...
# Function To Show Developer Options (Developer Only)
def dv_mode(code = 0):
    dvwin = tk.Toplevel(bg="white")
    dvwin.wm_title("Developer options")
    dvwin.wm_geometry("512x512")
    dvwin.resizable(False, False)
    dvwin.grab_set()
    dvwin.focus_set()
    heading = tk.Label(dvwin, text="Developer options", fg="black", bg="white", font=("Consolas", 35))
...
# Right-Click Menu
rm = tk.Menu(txt_edit, tearoff = 0)
rm.add_separator()
rm.add_command(label="Cut")
rm.add_command(label="Copy", command=copyup)
rm.add_command(label="Paste", command=pasteup)
rm.add_separator()
rm.add_command(label="Rename")
